 ...
 *
 * 作用：逐项检查 AutoX.js 在这台手机上能不能干活，结果弹 toast + 落一份报告文件。
 * 用法：AutoX.js 里导入本文件 → 点运行 → 看屏幕提示 + /sdcard/ghz_outbox/自检报告.txt
 */
...
function main() {
    ...
    if (auto.service == null) {
        toastLog("【X】无障碍服务没开！\n去 系统设置→辅助功能→无障碍→AutoX.js 打开开关，再重新运行本脚本");
    ...
    try {
        ...
        lines.push("文件写入: 失败 " + e3 + "（去系统设置给 AutoX.js 存储权限）");
        toastLog("【X】文件写入失败，给 AutoX.js 存储权限后重试");
    }
